...
router.get("/orders", async function (req, res) {
  try {
    ...
    if (user.role === "admin") {
      ...
      orders = await Order.find().populate("items.product").populate("userId").exec();
      return res.render("admin/orders", { orders, userRole: user.role });
    } else if (user.role === "seller") {
      // Seller: Fetch orders containing their products
      orders = await Order.find({
        "items.product.uploadedBy": user._id
      }).populate({
        path: "items.product",
        match: { uploadedBy: user._id }
      }).populate("userId").exec();
      
      return res.render("userprofile/orders", { orders });
    } else {
