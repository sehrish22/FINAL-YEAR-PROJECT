...
router.get("/", async function (req, res, next) {
  try {
    ...
    const pets = await Pet.find(filter).populate('uploadedBy');
    ...
    if (loggedInUserId) {
      userPets = pets.filter((pet) => pet.uploadedBy?._id.toString() === loggedInUserId.toString());
      otherPets = pets.filter((pet) => pet.uploadedBy?._id.toString() !== loggedInUserId.toString());
    } else {
